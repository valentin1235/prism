---
name: analyze
description: Multi-perspective agent team analysis with ontology-scoped investigation and MCP-based Socratic verification with mathematical ambiguity scoring. Use this skill for incident analysis, postmortem reports, outage investigation, root cause analysis, system analysis, or any deep multi-perspective investigation that requires verified findings with hallucination detection.
version: 4.1.0
user-invocable: true
...
Team size: 2 min analysts, no hard max (typically 3-5; complex cases may need more). Verification runs via MCP tools, not sidecar agents.
