...
**Primary input: Read the final report generated by analyze.** This report is the core source for post-processing.

1. Use `Glob` to find the report file inside `{ANALYZE_STATE_DIR}/` (filename pattern: `*report*`, `*Report*`, or the largest `.md` file in the directory)
2. `Read` the report file

**Secondary input: Obtain confidence scores** — try in this order:

1. Attempt to `Read` `{ANALYZE_STATE_DIR}/verification-log.json`
   - If exists → extract `weighted_total` and `verdict` per perspective
2. If file not found → parse the "Socratic Verification Summary" section from the analyze report
...
4. `{PRD_FILE_PATH}` — original PRD (for PRD citation verification)
5. `{ANALYZE_STATE_DIR}/perspectives.json` — perspective information (read if available)
6. `{ANALYZE_STATE_DIR}/verified-findings-*.md` — per-perspective detailed results (read if available)

### Step 2: Read Report Template

Find the report template relative to this prompt file:

```
../templates/report.md
```

If path is unknown, search via `Glob` for `**/prd-v3/templates/report.md`.
