...
func TestParseDAFindings(t *testing.T) {
	findings := parseDAFindings(sampleDAOutput)
...
func TestParseDASummary(t *testing.T) {
	confidence, topConcerns, whatHoldsUp := parseDASummary(sampleDAOutput)
...
`
	findings := parseDAFindings(input)
	if len(findings) != 0 {
...
func TestParseDAFindings_NoSections(t *testing.T) {
	findings := parseDAFindings("No structured output here")
	if len(findings) != 0 {
...
func TestFilterActionableFindings_Empty(t *testing.T) {
	actionable := filterActionableFindings(nil)
	if actionable != nil {
		t.Errorf("expected nil for nil input, got %v", actionable)
	}

	actionable = filterActionableFindings([]DAFinding{})
	if actionable != nil {
...
func TestFilterActionableFindings_IntegrationWithParsedOutput(t *testing.T) {
	...
	allFindings := parseDAFindings(sampleDAOutput)
	actionable := filterActionableFindings(allFindings)
...
`
	allFindings := parseDAFindings(input)
	actionable := filterActionableFindings(allFindings)
...
func TestSkipNoOp_ZeroFindings_SignalsNoChange(t *testing.T) {
	...
	allFindings := parseDAFindings(`## Challenged Framings
...
`)
	actionable := filterActionableFindings(allFindings)
	pass := shouldPassDA(countSeverities(actionable))
...
func TestShouldPassDA_EarlyTermination(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := shouldPassDA(tt.criticalCount, tt.majorCount)
			if got != tt.wantPass {
				t.Errorf("shouldPassDA(%d, %d) = %v, want %v",
...
// AC 6: countSeverities correctly tallies CRITICAL and MAJOR from findings
func TestCountSeverities(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			[]DAFinding{{Severity: "MINOR"}, {Severity: "MINOR"}},
			0, 0,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c, m := countSeverities(tt.findings)
			if c != tt.wantCritical || m != tt.wantMajor {
				t.Errorf("countSeverities() = (%d, %d), want (%d, %d)",
...
func TestEarlyTermination_EndToEnd(t *testing.T) {
	...
	allFindings := parseDAFindings(sampleDAOutput)
	actionable := filterActionableFindings(allFindings)
	crit, maj := countSeverities(actionable)
	pass := shouldPassDA(crit, maj)
...
- **What holds up**: Everything
`
	cleanFindings := parseDAFindings(cleanInput)
	cleanActionable := filterActionableFindings(cleanFindings)
	cleanCrit, cleanMaj := countSeverities(cleanActionable)
	cleanPass := shouldPassDA(cleanCrit, cleanMaj)
...
func TestMaxDARounds_Constant(t *testing.T) {
	if maxDARounds != 3 {
		t.Errorf("maxDARounds = %d, want 3", maxDARounds)
	}
}
...
func TestHardStop_TrueOnFinalRound(t *testing.T) {
	// When round == maxDARounds (3), hard_stop must be true
	hardStop := maxDARounds >= maxDARounds // simulates the check in handleDAReview
	if !hardStop {
		t.Error("hard_stop should be true when round == maxDARounds")
	}
	...
	result := DAReviewResult{
		...
		MaxRounds: maxDARounds,
		HardStop:  true,
		Findings:  []DAFinding{{Section: "Missing Perspectives", Title: "Gap", Severity: "CRITICAL", Concern: "big gap"}},
	}
	...
	if !parsed["hard_stop"].(bool) {
		t.Error("hard_stop must be true when round == maxDARounds")
	}
...
func TestHardStop_FalseBeforeMaxRound(t *testing.T) {
	...
	for _, round := range []int{1, 2} {
		...
		if hardStop {
			t.Errorf("round %d should not trigger hard_stop (maxDARounds=%d)", round, maxDARounds)
		}
	}
}

func TestHardStop_ExceedingMaxRoundsResult(t *testing.T) {
	// When round > maxDARounds, the handler returns a hard-stop result
...
func TestThreeRoundFailure_DAPassedFalse(t *testing.T) {
	...
	for round := 1; round <= maxDARounds; round++ {
		...
		_, err := PatchSeedAnalysisFile(seedPath, patch)
		if err != nil {
	...
	final, err := ReadSeedAnalysis(seedPath)
	if err != nil {
...
func TestThreeRoundFailure_NoUnresolvedFindingsRecorded(t *testing.T) {
	...
	result, err := PatchSeedAnalysisFile(seedPath, patch)
	if err != nil {
...
func TestThreeRoundFailure_WorkflowProceeds(t *testing.T) {
	...
	consumed, err := ReadSeedAnalysis(seedPath)
	if err != nil {
...
func TestThreeRoundFailure_HardStopResult(t *testing.T) {
	...
	result := DAReviewResult{
		...
		Round:         maxDARounds,
		MaxRounds:     maxDARounds,
		HardStop:      maxDARounds >= maxDARounds, // true
		Findings: []DAFinding{
	...
	if int(parsed["round"].(float64)) != maxDARounds {
		t.Errorf("round = %v, want %d", parsed["round"], maxDARounds)
	}
}

func TestThreeRoundFailure_ExceedsMaxReturnsEmpty(t *testing.T) {
	// If somehow round > maxDARounds, the tool returns immediately
	...
	result := DAReviewResult{
		...
		HardStop:  true,
		Findings:  []DAFinding{},
		RawOutput: fmt.Sprintf("hard stop: round %d exceeds maximum of %d rounds", maxDARounds+1, maxDARounds),
	}
...
func TestDAReviewResult_JSONStructure(t *testing.T) {
	// Verify the DAReviewResult marshals to expected JSON shape
	result := DAReviewResult{
		Pass:             false,
		CriticalCount:    1,
		MajorCount:       2,
		Findings:         []DAFinding{{Section: "Challenged Framings", Title: "Test", Severity: "CRITICAL"}},
